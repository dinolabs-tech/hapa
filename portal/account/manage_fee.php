	...
	<form id="manage-fees">
		...
		<input type="hidden" name="id" value="<?php echo isset($id) ? $id : '' ?>">
		 <div class="form-group">
            ...
            <input type="hidden" class="form-control" name="ef_no" value="<?php echo isset($ef_no) ? $ef_no :'' ?>" required>
        </div>
		<div class="form-group">
			...
			<select name="student_id" id="student_id" class="custom-select input-sm select2">
				...
				?>
				<option value="<?php echo $row['id'] ?>" <?php echo isset($student_id) && $student_id == $row['id'] ? 'selected' : '' ?>><?php echo ucwords($row['name']).' | '. $row['id'] ?></option>
				<?php endwhile; ?>
			</select>
		</div>
		<div class="form-group">
			<label for="" class="control-label">Fee Type</label>
			<select name="course_id" id="course_id" class="custom-select input-sm select2">
				<option value=""></option>
				<?php
					$student = $conn->query("SELECT *,concat(course,'-',level, '-', arm, '-', hostel) as class FROM courses order by course asc ");
					while($row= $student->fetch_assoc()):
				?>
...
<script>
    ...
    // Add this block to copy the student id into the ef_no field
    $('#student_id').change(function(){
        var studentId = $(this).val();
        $('[name="ef_no"]').val(studentId);
    })
