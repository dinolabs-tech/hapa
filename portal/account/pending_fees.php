					...
					<div class="table-responsive">
						<table class="table table-condensed table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
$fees = $conn->query("SELECT ef.*, s.name AS sname, s.id_no, (ef.total_fee - (SELECT IFNULL(SUM(amount), 0) FROM payments WHERE ef_id = ef.id)) AS balance FROM student_ef_list ef INNER JOIN student s ON s.id = ef.student_id HAVING balance > 0 ORDER BY s.name ASC");
								...
								<tr>
									...
									<td class="text-right">
										<p> <b><?php echo number_format($row['total_fee'] - $balance,2) ?></b></p>
